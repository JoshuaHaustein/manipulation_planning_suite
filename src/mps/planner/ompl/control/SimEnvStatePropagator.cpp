    ...
    ::ompl::base::State* result) const
{
    ...
    auto callback_fun = [this](sim_env::LinkPtr a, sim_env::LinkPtr b) { return _validity_checker->checkContact(a, b); };
    while (time < velocity_control->getMaxDuration() and propagation_success) {
        ...
        bool propagation_success = _world->stepPhysics(callback_fun);
    ...
    if (_semi_dynamic and propagation_success) {
        ...
        while (resting_time <= _t_max and not _world->atRest() and propagation_success) {
            propagation_success = _world->stepPhysics(callback_fun);
